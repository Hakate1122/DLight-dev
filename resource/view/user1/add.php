...
<body class="lmo-container">
    ...
    <div class="lmo-card" style="margin: 2rem auto; padding: 2rem; max-width: 640px;">
        <div class="lmo-row" style="justify-content: space-between; align-items: center; gap: 1rem; margin-bottom: 1rem;">
            <div>
                <p class="lmo-text-muted" style="margin-bottom: 0.5rem;">Tạo tài khoản mới</p>
                <h1 style="margin: 0; font-size: 2rem;">Thêm người dùng</h1>
            </div>
        ...
        <form class="lmo-form" action="<?= route('user.store') ?>" method="POST">
            <div class="lmo-form-row">
                <label class="lmo-label" for="name">Tên</label>
                <input class="lmo-input" type="text" id="name" name="name" value="<?= old('name') ?>" placeholder="Nhập tên người dùng">
            </div>

            <div class="lmo-form-row">
                <label class="lmo-label" for="email">Email</label>
                <input class="lmo-input" type="email" id="email" name="email" value="<?= old('email') ?>" placeholder="Nhập địa chỉ email">
            </div>

            <div class="lmo-form-actions" style="margin-top: 1.5rem;">
                <button type="submit" class="lmo-btn lmo-btn-primary">Lưu</button>
            </div>
        </form>
    </div>
